...
/* ── Fetch JSON — returns [] while loading, [] on error ── */
function useJSON(path) {
  ...
  useEffect(() => {
    fetch(path)
      ...
      .catch(()  => setData([]));              // empty array on any error
  }, [path]);
  return data;
}
...
/* ════════════════════════════════════════════════════════
   ...
   Sidebar layout: category menu on the left, note list on the right.
   ════════════════════════════════════════════════════════ */
const NOTE_TOPICS = [
...
function NotesPage({ onTitleClick, archive, setArchive, onOpen }) {
  ...
  if (notes === null) return <Loading />;

  // Active filter: null → "all"
  const active = archive || "all";

  // Filtered list
  const list = active === "all"         ? notes
             : active.startsWith("year:") ? notes.filter(n => n.date.startsWith(active.slice(5)))
             :                              notes.filter(n => n.topic === active);

  // Topics that actually have notes
  const presentTopics = NOTE_TOPICS.filter(t => notes.some(n => n.topic === t.id));
  ...
  return (
    <div className="page-enter">
      ...
      <h1 className="title-trigger" data-title="notes" onClick={onTitleClick}
          style={{ fontFamily:"var(--serif)", fontWeight:500, fontSize:56, margin:"0 0 32px 0", lineHeight:1.05 }}>
      ...
      {notes.length === 0 ? (
        ...
        <div className="notes-layout">
          ...
          <aside className="notes-sidebar">
            <SidebarBtn active={active === "all"} onClick={() => setArchive(null)}>
              <span>All notes</span>
            ...
            {presentTopics.map(t => {
              ...
              return (
                <SidebarBtn key={t.id} active={active === t.id} onClick={() => setArchive(t.id)}>
                  <span><span style={{ marginRight:6, opacity:0.6 }}>{t.glyph}</span>{t.label}</span>
            ...
            {years.map(y => {
              ...
              const id = "year:" + y;
              return (
                <SidebarBtn key={y} active={active === id} onClick={() => setArchive(id)}>
                  <span>{y}</span>
                  <span style={{ opacity:0.5 }}>{ct}</span>
                </SidebarBtn>
              );
